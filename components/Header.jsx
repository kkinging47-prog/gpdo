...
export default function Header() {
  ...
  return <header className="site-header"><div className="container nav">
    ...
    <nav className={`nav-links${open ? ' open' : ''}`} aria-label="Primary navigation">{links.map(([href,label]) => <Link key={href} href={href} className={pathname === href || (href !== '/' && pathname?.startsWith(href + '/')) ? 'active' : ''} onClick={() => setOpen(false)}>{label}</Link>)}<Link className="nav-cta" href="/get-involved#support" onClick={() => setOpen(false)}>Support GPDO</Link></nav>
